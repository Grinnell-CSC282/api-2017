...
ali2int (ALInt * i)
{
  ...
  if (errno)
	...
	{
	  printf
	    ("range error: could not convert ALInt %s to int\nreturning 0 instead\n",
	     s);
	  l = 0;
	}
    }
...
str2ali (char *s)
{
  if (!isInt_str (s))
    {
      printf
	("Unable to convert string %s to ALInt: It must contain only digits and a minus sign in leftmost position\n",
	 s);
      return NULL;
    }
  ...
  if (tail < head)
    {
      printf
	("Error converting string to ALInt: Argumant is not string of integer\nreturning 0 instead\n");
      return NULL;
    }
  if (head[0] == '-')
    {
  ...
  while (tail >= head)
	...
	{
	  current->next =
	    newnode (current, ((unsigned int) atoi (bufferhead)));
...
intnode *
newnode (intnode * previous, unsigned int value)
{
...
ALInt *
removeLeadingZeros_ali (ALInt * i)
{
...
unsigned int
uintpow (unsigned int base, unsigned int exp)
{
  unsigned int x = 1;
  while (exp)
    {
...
unsigned int
getNthComponent (ALInt * x, unsigned int n)
{
  unsigned int p = 0;
  intnode *current = x->head;
...
void
multiplyByBASEtopower (ALInt * x, unsigned int powerForBase)
{
  if (powerForBase > 0)
    {
...
main ()
{
  printf ("INT_MAX = %d\nLINT_MAX = %ld\n", INT_MAX, LONG_MAX);
  char *x[] =
    { "0", "384392", "34958340965023472", "-329576232342",
"-849275028347590387402", "-3482345723952382936459", "-00000010000000000000",
