...
ALIntDigit *
str2ali_k (char *str, ALIntDigit * last, int *length, ALIntDigit ** tail)
{
  if (!str[0])
    {
      *tail = last;
...
ali2int (ALInt * a)
{
  ...
  while (sum < INT_MAX && sum >= 0)
    {				// If integer overflow, sum will go negative
      if (!cur)
	return (a->nonnegative ? sum : -sum);
